...
def run_demo():
    ...
    print("\n--- 1. Creating Products ---")
    p1_response = requests.post(f"{BASE_URL}/products", json=product1_data)
    p2_response = requests.post(f"{BASE_URL}/products", json=product2_data)
    ...
    all_products_resp = requests.get(f"{BASE_URL}/products")
    ...
    update_resp = requests.put(f"{BASE_URL}/products/{p1_id}", json=update_data)
    ...
    all_orders_resp = requests.get(f"{BASE_URL}/orders")
    ...
    status_resp = requests.patch(f"{BASE_URL}/orders/{order_id}/status", json=status_data)
    ...
    if cleanup == "yes":
        ...
        # Delete Order
        delete_order_resp = requests.delete(f"{BASE_URL}/orders/{order_id}")
        if delete_order_resp.status_code == 204:
            print(f"Successfully deleted Order {order_id}")
        
        # Delete Products
        for pid in [p1_id, p2_id]:
            delete_prod_resp = requests.delete(f"{BASE_URL}/products/{pid}")
